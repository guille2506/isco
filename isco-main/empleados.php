...
//0- determinar cantidad cursos enrolar
$url = $_SESSION['url'].'v1/index.php/insco_usuario_cursos_multiples';
...
*/
   $cuerpo="";
   $i=1;
   if ( ( count($res["curso_activo"]) > 1) && ($_SESSION['cursos']=="") ) {
...
*/
 ...
 $parametros="curso=".$curso_enrolado."&tema=".$_SESSION['tema_cronograma'];
 //echo $parametros; //die();
 $ch = curl_init();
 curl_setopt($ch, CURLOPT_URL, $url);
 curl_setopt($ch, CURLOPT_TIMEOUT, 30);
    ...
    $i=1;
     ...
     foreach($res['items'] as $items_tema) {
        if ($items_tema['name']=="book"){
            ...
            $parametros="course=".$_SESSION['curso']."&id=".$_SESSION['tema_cronograma'];
            //echo $parametros; //die();
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_TIMEOUT, 30);
              ...
              <div class="tab-content">
                <div class="tab-pane preview-tab-pane active" role="tabpanel" aria-labelledby="tab-dom-ebd7cae0-f4cb-4425-91da-b656fcb40d10" id="dom-ebd7cae0-f4cb-4425-91da-b656fcb40d10">
                  <div class="table-responsive scrollbar">
                    <?php
                      $url = $_SESSION['url'].'v1/index.php/insco_total_modulo_cursos';    
                      $parametros="curso=".$_SESSION['curso'];
                      $res_total_modulos = resulrow($url, $parametros);  
                      $total_modulos=0;
                    foreach($res_total_modulos['total_modulos'] as $quiz) {
                      $total_modulos=$quiz['total']; 
                    }
                    $url = $_SESSION['url'].'v1/index.php/insco_total_modulo_completados_cursos';    
                    $parametros="curso=".$_SESSION['curso'] ."&usuario=".$_SESSION['idmoodle'];
                    $res_total_completado = resulrow($url, $parametros);  
                    $total_completo=0;
                    foreach($res_total_completado['total_completo'] as $quiz) {
                    $total_completo=$quiz['total_completo']; 
                    }   
                    //echo "valores ".$total_completo;
                    $porcentaje_completo = $total_completo * 100 / $total_modulos;
                    $porcentaje_incompleto= 100 -$porcentaje_completo;
                    ?>
                      ...
                      <tr class="border-bottom border-200">
                          ...
                          <td class="align-middle pe-x1">
                            <div class="d-flex align-items-center">
                              <div class="progress me-3 rounded-3 bg-200" style="height: 5px; width:80px;" role="progressbar" aria-valuenow="<?php echo round($porcentaje_completo,2);?>" aria-valuemin="0" aria-valuemax="100">
                                <div class="progress-bar rounded-pill" style="width: <?php echo round($porcentaje_completo,2);?>%;"></div>
                              </div>
                              <div class="fw-semi-bold ms-2"><?php echo round($porcentaje_completo,2);?>%</div>
                            </div>
                          </td>
                        </tr>
       ...
       <div class="row g-0">
            <div class="col-sm-6 col-xxl-3 pe-sm-2 mb-3 mb-xxl-0">
              <div class="card">
                ...
                <?php
                // grabar en pendiente como visto
    $url = $_SESSION['url'].'v1/index.php/gesinpol_pendientes';    
    $parametros="user=".$_SESSION['idmoodle']."&curso=".$_SESSION['curso'];
    //echo $parametros;
    $res_vistos = resulrow($url, $parametros);  
    $curso_nombre="";
   foreach($res_vistos['pendientes'] as $quiz) {
    $temario=$quiz['temario'];
    $url=trim($quiz['url']);
?>
                  <div class="d-flex align-items-center position-relative mb-3">
                    ...
                    <div class="flex-1 ms-3">
                      <h6 class="mb-0 fw-semi-bold">
                        <a class="stretched-link text-900" href="#"><?php echo $temario ?></a>
                      </h6>                      
                    </div>
                  </div>
                  <?php }?> 
            ...
            <div class="col-sm-6 col-xxl-3 ps-sm-2 order-xxl-1 mb-3 mb-xxl-0">
              <div class="card h-100">
                ...
                <div class="card-body py-2">
                <?php
               $url = $_SESSION['url'].'v1/index.php/insco_sig_meet_cursos';    
               $parametros="curso=".$_SESSION['curso'];
               $res_sig = resulrow($url, $parametros);  
           
               $itempdf=3;
               $itemvideo=3;
               $item=0;
           
              foreach($res_sig['sig_modulos'] as $quiz) {
               $temario=$quiz['name'];        
               $tipo=$quiz['tipo'];
               $fecha=$quiz['fecha'];    
               $url_meet=$quiz["url"];
                ?>             
                  <div class="d-flex align-items-center position-relative mb-3">
                    <div class="avatar avatar-2xl status-online">
                    <?php if ($tipo=="pdf"){ ?>
                      <img class="rounded-circle" src="assets/img/otro_icons/pdf.png" alt="">
                      <?php } ?>
                      <?php if ($tipo=="video"){ ?>
                      <img class="rounded-circle" src="assets/img/otro_icons/video.png" alt="">
                      <?php } ?>
                      <?php if ($tipo=="meet"){ ?>
                      <img class="rounded-circle" src="assets/img/otro_icons/Google_Meet_icon_(2020).svg.png" alt="">
                      <?php } ?>
                    </div>
                    <div class="flex-1 ms-3">
                      <h6 class="mb-0 fw-semi-bold">
                        <a class="stretched-link text-900" href="<?php echo $url_meet;?>">
                        <?php if ($tipo=="meet"){ ?>  
                        <?php echo $temario ?> : <?php echo $fecha ?>
                        <?php }else{ ?>
                ...
                <div class="card-footer bg-light p-0"><a class="btn btn-sm btn-link d-block w-100 py-2" href="<?php echo $_SESSION['clase_online'];?>">Video Clase online<svg class="svg-inline--fa fa-chevron-right fa-w-10 ms-1 fs--2" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="chevron-right" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" data-fa-i2svg=""><path fill="currentColor" d="M285.476 272.971L91.132 467.314c-9.373 9.373-24.569 9.373-33.941 0l-22.667-22.667c-9.357-9.357-9.375-24.522-.04-33.901L188.505 256 34.484 101.255c-9.335-9.379-9.317-24.544.04-33.901l22.667-22.667c9.373-9.373 24.569-9.373 33.941 0L285.475 239.03c9.373 9.372 9.373 24.568.001 33.941z"></path></svg><!-- <span class="fas fa-chevron-right ms-1 fs--2"></span> Font Awesome fontawesome.com --></a></div>
              </div>
            </div>
       <!-- end  ultimo visto -->   
		  <div class="card mb-3">
