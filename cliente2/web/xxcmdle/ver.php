                ...
                <ol class="dd-list">
                    <li class="dd-item" data-id="1">
                        <div class="dd-handle d-flex justify-content-between align-items-center">
                            ...
                            <div class="action">
                                <?php if ($_GET["tipo"]==8){ ?>
                                <a href="area_estudio_ajax.php?curso=<?PHP echo $courseid; ?>&cat=<?PHP echo $cat; ?>"><i class="zmdi zmdi-close" style="color: #000 !important"></i></a>
                                <?php }else{ ?>
                                <a href="simulacro.php?tipo=<?PHP echo $_GET["tipo"]; ?>&op=m"><i class="zmdi zmdi-close"></i></a>
                                <?php } ?>
                            </div>
                        </div>
...
?>
<div class="row clearfix"><div class="col-2 position-fixed reloj"><span><label id="countdown"></label></span></div>
<div class="col-12" align="center"><h4><strong><?PHP echo $_POST["name"]; ?></strong></h4></div></div>

<div id="detalletpreg" class="row clearfix">
<?PHP 
//$t=1;
$ur2 = $uri.'v2/index.php/gesinpol_quiz_fixed_attempt_question_showl'; 
$parametro2 = "quizfixedid=".$quizfixedid."&attempt=".$attempt."&userid=".$userid."&pag=".$idp."&maxpreg=".$maxpreg;
	...
	<div id="<?PHP echo $idp; ?>" class="col-md-3 col-lg-3 text-letf">
        <div class="card">
            <div class="body" style="color: #004C45">                            
                <h3><strong>Pregunta <?PHP echo $idp; ?></strong></h3>
                <div class="bg-warning" style="padding-block: 10px">
                    <div id="resultado_msg_<?PHP echo $idp; ?>" class="text" style="margin: 10px;"><strong><?PHP if (isset($tupla2["answerid"])) {$respondida = "Respuesta guardada";} echo $respondida; ?></strong></div>
...
?>            
	...
	$tuplq2 = resulrow($ur3, $parametro3);
	
	$card=1; $fraction=0.00; $blanco=0; $ss=''; $cont=1; $c1=$c=$cc=$p=0; 
		  $r=NULL; $a=0; $auxid=0; $idjs=""; $qualify=0;
		  $resp='<div class="alert alert-danger"><strong>Respuesta Incorrecta.</strong></div>';
			
	if($tupla2["qtype"]=="multichoice"){
			
			if ($sum==1){
			foreach($tuplq2["quizrespuesta"] as $tupla55)
			{
			...
			foreach($tuplq2["quizrespuesta"] as $tupla4)
			{
	...
	if ($ropcion == 2){
		$s=''; $imgr=''; 
		if($tupla2["answerid"]==NULL){}
		elseif ($tupla4["id"]==$tupla2["answerid"]) {
	...
	}elseif ($tupla4["id"]==$tupla2["answerid"]) {$s=' checked="checked"'; $ss='document.getElementById("e'.$idp.'a").style.display = "block";';
	}else{$s='';} 
	?>        
            <div class="radio">
    ...
    <?php 	}else{	
				if ($ropcion == 2){
				  ...
				  $tr=0;
				  foreach($tuplq2["quizrespuesta"] as $tupla66){	$tr++; }
				}
				$blanco=1;
...
?>        
    ...
    <?PHP } //hasta aqui ?>
	   ...
	   <?PHP if($tupla2["answerid"]==NULL){} else { ?>
          <button class="btn btn-success l-turquoise waves-effect" type="button" data-toggle="collapse" data-target="#collapsePreg<?PHP echo $tupla2["sequencenumber"]; ?>" aria-expanded="false" aria-controls="collapseExample"> MOSTRAR Solución </button>
            <div class="collapse" id="collapsePreg<?PHP echo $tupla2["sequencenumber"]; ?>">
            <div class="panel-footer">
               <div class="feedback">
                  ...
                  <div class="generalfeedback"><?PHP echo $tupla2["generalfeedback"]; ?></div>
               </div>
            </div>
            </div>
       <?PHP } ?>
